...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings['url']:
                if clicked == typed:
                    ...
                    await query.answer(url=f"https://t.me/Poster_Flix_Bot?u={me.username}&start=short_{file_id}")
                    return
                else:
                    await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
            else:
                if clicked == typed:
                    await query.answer(url=f"https://t.me/Poster_Flix_Bot?u={me.username}&start={ident}_{file_id}")
                    return
                else:
                    await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
        ...
        try:
                if settings['url']:
                    await query.answer(url=f"https://t.me/Poster_Flix_Bot?u={me.username}&start=sendfiles1_{key}")
                else:
                    await query.answer(url=f"https://t.me/Poster_Flix_Bot?u={me.username}&start=allfiles_{key}")    
                
        except UserIsBlocked:
            await query.answer('Uɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ ᴍᴀʜɴ !', show_alert=True)
        except PeerIdInvalid:
            await query.answer(url=f"https://t.me/Poster_Flix_Bot?u={me.username}&start=sendfiles3_{key}")
        except Exception as e:
            logger.exception(e)
            await query.answer(url=f"https://t.me/Poster_Flix_Bot?u={me.username}&start=sendfiles4_{key}")
